...
scores = { "task" => 85, "comment" => 68, "message" => 100, "product" => 98, "item" => 72, "account" => 50, "report" => 86 }

puts "All scores:"
scores.each { |name, score| puts "  \#{name.ljust(12)} \#{score}" }

passing  = scores.select { |_, v| v >= 71 }
failing  = scores.reject { |_, v| v >= 71 }
top      = scores.max_by { |_, v| v }
lowest   = scores.min_by { |_, v| v }
average  = scores.values.sum.to_f / scores.size

puts "Passing (>=71)  : \#{passing.keys.inspect}"
...
merged = scores.merge({ "bonus_entry" => 67 }) { |_, old, new_val| [old, new_val].max }
