...
class BookStore
  ...
  attr_reader :session, :score

  def initialize(session, score)
    @session = session
    @score = score.to_f
  end
...
items = [
  BookStore.new("session_a", 37),
  BookStore.new("session_b", 66),
  BookStore.new("session_c", 116),
  BookStore.new("session_d", 24),
]
