...
scores = { "entry" => 53, "session" => 76, "comment" => 63, "payment" => 59, "invoice" => 73, "post" => 72, "record" => 63 }

puts "All scores:"
scores.each { |name, score| puts "  \#{name.ljust(12)} \#{score}" }

passing  = scores.select { |_, v| v >= 60 }
failing  = scores.reject { |_, v| v >= 60 }
top      = scores.max_by { |_, v| v }
lowest   = scores.min_by { |_, v| v }
average  = scores.values.sum.to_f / scores.size

puts "Passing (>=60)  : \#{passing.keys.inspect}"
