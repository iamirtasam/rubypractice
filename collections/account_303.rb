...
scores = { "message" => 72, "report" => 55, "payment" => 58, "invoice" => 81, "entry" => 89, "token" => 65, "account" => 79 }

puts "All scores:"
scores.each { |name, score| puts "  \#{name.ljust(12)} \#{score}" }

passing  = scores.select { |_, v| v >= 64 }
failing  = scores.reject { |_, v| v >= 64 }
top      = scores.max_by { |_, v| v }
lowest   = scores.min_by { |_, v| v }
average  = scores.values.sum.to_f / scores.size

puts "Passing (>=64)  : \#{passing.keys.inspect}"
...
merged = scores.merge({ "bonus_entry" => 59 }) { |_, old, new_val| [old, new_val].max }
