...
scores = { "event" => 97, "comment" => 65, "product" => 83, "rating" => 55, "entry" => 63, "account" => 89, "invoice" => 83 }

puts "All scores:"
scores.each { |name, score| puts "  \#{name.ljust(12)} \#{score}" }

passing  = scores.select { |_, v| v >= 68 }
failing  = scores.reject { |_, v| v >= 68 }
top      = scores.max_by { |_, v| v }
lowest   = scores.min_by { |_, v| v }
average  = scores.values.sum.to_f / scores.size

puts "Passing (>=68)  : \#{passing.keys.inspect}"
...
merged = scores.merge({ "bonus_entry" => 62 }) { |_, old, new_val| [old, new_val].max }
