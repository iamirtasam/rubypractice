...
scores = { "product" => 79, "task" => 54, "review" => 91, "record" => 89, "token" => 65, "category" => 100, "session" => 75 }

puts "All scores:"
scores.each { |name, score| puts "  \#{name.ljust(12)} \#{score}" }

passing  = scores.select { |_, v| v >= 76 }
failing  = scores.reject { |_, v| v >= 76 }
top      = scores.max_by { |_, v| v }
lowest   = scores.min_by { |_, v| v }
average  = scores.values.sum.to_f / scores.size

puts "Passing (>=76)  : \#{passing.keys.inspect}"
...
merged = scores.merge({ "bonus_entry" => 66 }) { |_, old, new_val| [old, new_val].max }
