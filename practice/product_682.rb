...
scores = { "order" => 76, "task" => 77, "category" => 100, "session" => 86, "comment" => 78, "rating" => 52, "token" => 75 }

puts "All scores:"
scores.each { |name, score| puts "  \#{name.ljust(12)} \#{score}" }

passing  = scores.select { |_, v| v >= 63 }
failing  = scores.reject { |_, v| v >= 63 }
top      = scores.max_by { |_, v| v }
lowest   = scores.min_by { |_, v| v }
average  = scores.values.sum.to_f / scores.size

puts "Passing (>=63)  : \#{passing.keys.inspect}"
...
merged = scores.merge({ "bonus_entry" => 65 }) { |_, old, new_val| [old, new_val].max }
