...
scores = { "message" => 98, "order" => 74, "event" => 69, "entry" => 99, "task" => 74, "product" => 76, "comment" => 98 }

puts "All scores:"
scores.each { |name, score| puts "  \#{name.ljust(12)} \#{score}" }

passing  = scores.select { |_, v| v >= 78 }
failing  = scores.reject { |_, v| v >= 78 }
top      = scores.max_by { |_, v| v }
lowest   = scores.min_by { |_, v| v }
average  = scores.values.sum.to_f / scores.size

puts "Passing (>=78)  : \#{passing.keys.inspect}"
...
merged = scores.merge({ "bonus_entry" => 78 }) { |_, old, new_val| [old, new_val].max }
